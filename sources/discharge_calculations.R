...
for (i in seq_along(dates1)) {
  ...
  tseq <- tibble(timestamp = seq(from = ev_start, to = ev_end, by = 60))
  ev_wb    <-  dat1 %>%
    filter(timestamp > ev_start & timestamp < ev_end)
  ev_cr6   <- dat2 %>%
    filter(timestamp > (ev_start - minutes(10)) & timestamp < (ev_end + minutes(10)))
  ...
  df_list[[i]] <- tseq %>%
    left_join(ev_cr6, by = "timestamp") %>%
    full_join(ev_wb, by = "timestamp")
}
